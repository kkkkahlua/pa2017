...
//	printf("%d %d %lld\n", sign, exp, sig_grs);
	...
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		...
		if(exp >= 0xff) {
			...
			overflow = true;
			if (!sign) return 0x7f800000;
			else return 0xff800000;
		}
		if(exp == 0) {
	...
	if(!overflow) {
		...
		uint32_t grs = sig_grs & 0x7, flag;
		if (grs > 4) flag = 1;
		else if (grs == 4) flag = 0;
		else flag = -1;
		sig_grs >>= 3;
		if (flag == 1 || (flag == 0 && sig_grs & 1)) ++sig_grs;
		if ((sig_grs >> 23) > 1) {
			sig_grs >>= 1;
			++exp;
		}
...
uint32_t internal_float_mul(uint32_t b, uint32_t a) {
	...
	uint32_t exp_res = fa.exponent + fb.exponent - 127 - 23 + 3;
