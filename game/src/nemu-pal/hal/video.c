...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, 
	...
	for (int i = 0; i < h; ++i) {
		int srcpos = (sy + i) * src->w + sx;
		int dstpos = (dy + i) * dst->w + dx;
		memcpy((char*)dst->pixels + dstpos, (char*)src->pixels + srcpos, w);
	}
...
/*	int16_t srcup = srcrect->y, srcleft = srcrect->x,
		srcdown = srcup + srcrect->h, srcright = srcleft + srcrect->w;
	int16_t dstup = dstrect->y, dstleft = dstrect->x,
		dstdown = dstup + srcrect->h < dst->h ? dstup + srcrect->h : dst->h,
		dstright = dstleft + srcrect->w < dst->w ? dstleft + srcrect->w : dst->w;
	int srci, srcj, dsti = dstup, dstj = dstleft;
	for (srci = srcup; srci < srcdown; ++srci) {
		for (srcj = srcleft; srcj < srcright; ++srcj) {
			int srcpos = srci * src->w + srcj;
			int dstpos = dsti * dst->w + dstj;
			dst->pixels[dstpos] = src->pixels[srcpos];
			++dstj;
			if (dstj == dstright) break;
		}
		++dsti; dstj = dstleft;
		if (dsti == dstdown) break;
	}*/
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
	...
	for (int i = 0; i < h; ++i) {
		for (int j = 0; j < w; ++j) {
			int dstpos = (dy + i) * dst->w + dx + j;
			*((char*)(dst->pixels) + dstpos) = color;
		}
	}
/*	int16_t dstup = dstrect->y, dstleft = dstrect->x,
		dstdown = dstup + dstrect->h < dst->h ? dstup + dstrect->h : dst->h,
		dstright = dstleft + dstrect->w < dst->w ? dstleft + dstrect->w : dst->w;
	int dsti, dstj;
	for (dsti = dstup; dsti < dstdown; ++dsti) {
		for (dstj = dstleft; dstj < dstright; ++dstj) {
			int dstpos = dsti * dst->w + dstj;
